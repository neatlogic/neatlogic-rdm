...
@Component
public class BugReopenTotalStatHandler extends BugCustomAttrStatHandlerBase implements IIssueStatHandler {
    @Override
    public String getStatKey() {
        return AppStatAttrType.BUG_REOPEN_TOTAL.getValue();
    }

    @Override
    protected String getTargetStatKey() {
        return AppStatAttrType.BUG_REOPEN_COUNT.getValue();
    }

    @Override
    public IssueStatResultVo calculate(IssueStatContextVo context) {
        return calculateCountGreaterThanZero(context, "重开缺陷数");
    }
}
