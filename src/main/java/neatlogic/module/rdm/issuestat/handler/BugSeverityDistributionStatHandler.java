...
@Component
public class BugSeverityDistributionStatHandler extends BugCustomAttrStatHandlerBase implements IIssueStatHandler {
    @Override
    public String getStatKey() {
        return AppStatAttrType.BUG_SEVERITY_DISTRIBUTION.getValue();
    }

    @Override
    protected String getTargetStatKey() {
        return AppStatAttrType.BUG_SEVERITY.getValue();
    }

    @Override
    public IssueStatResultVo calculate(IssueStatContextVo context) {
        return calculateDistribution(context, "严重程度分布");
    }
}
