...
@Component
public class StoryHighRiskStatHandler extends IssueStatHandlerBase implements IIssueStatHandler {
    @Override
    public String getStatKey() {
        return AppStatAttrType.STORY_HIGH_RISK.getValue();
    }

    @Override
    public IssueStatResultVo calculate(IssueStatContextVo context) {
        IssueStatResultVo resultVo = createResult(context, "metric", "高风险需求数");
        resultVo.setValue(issueMapper.getIssueOverviewHighRiskCount(toIssueVo(context)));
        return resultVo;
    }
}
